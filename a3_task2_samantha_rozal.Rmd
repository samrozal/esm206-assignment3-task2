...
(NOTE: Didn't get a warning message)

```{r}
# Your code here: 
sw_sep_name <- starwars %>% 
  separate(col = name,
...
```{r}
# Your code here: 
ymd("2001/10/26") #my younger brother's birthday :)
...
storms_danger <- storms %>%
  mutate(danger = case_when(status == "hurricane" ~ "danger",
                            TRUE ~ "moderate"))
